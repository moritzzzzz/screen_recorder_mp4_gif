...
<img width="840" height="1094" alt="image" src="https://github.com/user-attachments/assets/6800e3d8-748e-4fd3-a29a-f5f8379a9ec6" />
...
## Uploading to a New GitHub Repo

### Files to include

Everything in this repo *except* build artifacts:
...
- `Scripts/build.sh`
- A `LICENSE` file of your choice (MIT, Apache-2.0, etc.)

### Files to exclude

Already covered by the included `.gitignore`:

- `.build/` — Swift Package Manager build directory
- `.swiftpm/` — SPM local state
- `ScreenRecorder.app/` — built application bundle (others build their own)
- `Package.resolved` — optional; safe to include if you later add dependencies
- `.DS_Store`, `*.xcuserstate`, etc.
- `.claude/` — local editor/agent state

### One-time setup

```sh
cd /Volumes/Crucial/Archive/custom_screen_recorder
git init
git add .
...
Others can then clone and build with:
